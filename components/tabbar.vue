  ...
  <view>
    ...
    <view class="tabbar" v-if="list.length > 1">
      <view class="navigator">
        ...
        >
					<i class="iconfont " :class="[`icon-${item.icon}`,{'primary': selectedIndex === index}]"></i>
          <text
            :class="['item-text', { 'primary': selectedIndex === index }]"
            >{{ item.text }}</text
          >
        </view>
      </view>
...
export default {
  data() {
	...
	computed:{
		selectedIndex() {
			return uni.getStorageSync("selectedIndex") || 0
		}
	},
  methods: {
    setTabbar() {
